...
const Inventory = () => {
  ...
  useEffect(() => {
    const carregarResumoEMarcas = async () => {
      setLoadingMarcas(true);

      try {
        const [{ count, error: countError }, { data, error: marcasError }] =
          await Promise.all([
            supabase
              .from("vehicles_clean")
        ...
        console.error("Erro ao carregar estoque do administrador:", error);
      } finally {
        setLoadingMarcas(false);
      }
    };

    carregarResumoEMarcas();
  }, []);
  ...
  useEffect(() => {
    const carregarModelos = async () => {
      ...
      try {
        ...
        console.error("Erro ao carregar modelos do estoque:", error);
      } finally {
  ...
  const buscarVeiculos = async () => {
    ...
    try {
      ...
      if (encontrados.length === 0) {
        setMensagem("Nenhum veículo disponível encontrado com esse filtro.");
      }
    } catch (error) {
      console.error("Erro ao buscar veículos do estoque:", error);
      setMensagem("Não foi possível consultar o estoque agora.");
    } finally {
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-2">
            Consulte o estoque real da loja com base nos veículos disponíveis no
            Revenda Mais. Esta tela é somente leitura.
          </p>
        </div>
        ...
        <CardContent className="p-6 md:p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-zinc-500">
                Lista real das marcas disponíveis no estoque.
              </p>
            </div>
