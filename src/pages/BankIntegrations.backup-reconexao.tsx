...
const BankIntegrations = () => {
  ...
  const carregarStatusBancos = async () => {
    try {
      const { data, error } = await supabase
        .from('credenciais_lojista')
        .select('nome_banco, status_conexao, is_active')
        .not('nome_banco', 'is', null);

      if (error) throw error;

      setBanks(prev => prev.map(bank => {
        const conectado = data?.some((cred: any) =>
          cred.nome_banco === bank.name &&
          cred.is_active === true &&
          cred.status_conexao === 'conectado'
        );
      ...
      showError("Erro ao carregar status dos bancos: " + error.message);
    }
  };

  useEffect(() => {
    carregarStatusBancos();
  }, []);
  ...
  const handleDisconnect = async (bank: Bank) => {
    setIsLoading(true);
    ...
    try {
      const { error } = await supabase
        ...
        .eq('nome_banco', bank.name);

      if (error) throw error;

      setBanks(prev => prev.map(b =>
        b.id === bank.id ? { ...b, status: 'Pendente' } : b
      ));

      showSuccess(`${bank.name} desconectado com sucesso.`);
    } catch (error: any) {
      showError("Erro ao desconectar banco: " + error.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const WEBHOOK_URL = "https://semidefensively-hymnological-elvia.ngrok-free.dev/webhook/integracao-bancos";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {banks.map((bank) => (
          <Card key={bank.id} className="bg-[#0a0a0a] border-zinc-800 hover:border-[#d4af37]/50 transition-all duration-300 group">
            ...
            <CardContent>
                            <Button
                onClick={() => bank.status === 'Conectado' ? handleDisconnect(bank) : handleConfigure(bank)}
                disabled={isLoading}
                className={
                  bank.status === 'Conectado'
                    ? "w-full border-2 border-orange-500 bg-transparent text-orange-500 hover:bg-orange-500/10 rounded-xl font-bold py-6 transition-all"
                    : "w-full bg-zinc-900 hover:bg-zinc-800 text-white border border-zinc-800 rounded-xl font-bold py-6 transition-all"
                }
              >
                {bank.status === 'Conectado' ? "Desconectar" : "Configurar Acesso"}
              </Button>
            </CardContent>
          </Card>
        ))}
